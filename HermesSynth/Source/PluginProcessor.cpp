...
void HermesSynthAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int i = 0; i < synth.getNumVoices(); ++i)
    {
        if (auto voice = dynamic_cast<SynthVoice*>(synth.getVoice(i)))
        {
            ...
            voice->getOscillator1().setOscPitch(osc1Pitch);
            ...
            voice->getOscillator2().setOscPitch(osc2Pitch);
...
juce::AudioProcessorValueTreeState::ParameterLayout HermesSynthAudioProcessor::createParams()
{
    ...
    // OSC Voices
    params.push_back(std::make_unique<juce::AudioParameterInt>(juce::ParameterID { "VOICES", 1 }, "Voices", 1, 8, 1));
    ...
    params.push_back (std::make_unique<juce::AudioParameterInt>(juce::ParameterID{ "OSC2PITCH", 1 }, "Oscillator 2 Pitch", -48, 48, 12));
